  ...
  var patch = function(parent, node, oldVNode, newVNode, listener, isSvg) {
    if (oldVNode === newVNode) ; else if (
      ...
      while (newHead <= newTail && oldHead <= oldTail) {
        if (
          (oldKey = getKey$1(oldVKids[oldHead])) == null ||
          oldKey !== getKey$1(newVKids[newHead])
        ) {
          break
        }
      ...
      while (newHead <= newTail && oldHead <= oldTail) {
        if (
          (oldKey = getKey$1(oldVKids[oldTail])) == null ||
          oldKey !== getKey$1(newVKids[newTail])
        ) {
          break
        }
      ...
      if (oldHead > oldTail) {
        ...
        while (newHead <= newTail) {
          oldKey = getKey$1((oldVKid = oldVKids[oldHead]));
          newKey = getKey$1(
          ...
          if (
            newKeyed[oldKey] ||
            (newKey != null && newKey === getKey$1(oldVKids[oldHead + 1]))
          ) {
            if (oldKey == null) {
        ...
        while (oldHead <= oldTail) {
          if (getKey$1((oldVKid = oldVKids[oldHead++])) == null) {
            node.removeChild(oldVKid.node);
          }
        }
